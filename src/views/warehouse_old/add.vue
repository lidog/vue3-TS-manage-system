<template>
  <div class="filter flex flex-row items-center justify-between pb-5">
    <div class="w-2/3 flex flex-row items-center">
      <el-input class="mr-2" v-model="input" placeholder="扫码入库" clearable>
        <template #append>
      ...
      <el-button
        ...
        >入库</el-button
      >
      <el-upload :auto-upload="false" style="height: 32px; margin-left: 5px;" :on-success="drawer2=true">
        <el-button el-button type="primary">excel入库</el-button>
      </el-upload>
    </div>
    <div class="tools">
      <el-button
  ...
  >
    ...
    <el-table-column
      ...
      width="150"
      sortable
      :filters="filterMap.number"
    />
    <el-table-column
      prop="color"
      label="颜色"
      sortable
      :filters="filterMap.color"
    />
    <el-table-column
      prop="count"
      label="数量"
      sortable
      :filters="filterMap.count"
    />
    <el-table-column
      prop="kuNumber"
      label="库位"
      sortable
      :filters="filterMap.kuNumber"
    />
    <el-table-column
      prop="belongTo"
      label="所属成品"
      sortable
      :filters="filterMap.belongTo"
    />
    <el-table-column
      prop="boxNumber"
      label="箱号"
      sortable
      :filters="filterMap.boxNumber"
    />
    <el-table-column
      prop="operator"
      label="操作人"
      sortable
      :filters="filterMap.operator"
    />
  </el-table>
  <div class="my-drawer">
    <el-drawer v-model="drawer" title="添加产品">
      <el-main class="main flex-col justify-around">
        <remoteSelect placeholder="出入库类型" />
        <remoteSelect placeholder="条码编号" />
        <remoteSelect placeholder="产品编号" />
        <remoteSelect placeholder="颜色" />
        <remoteSelect placeholder="数量" />
        <remoteSelect placeholder="库位" />
        <remoteSelect placeholder="所属成品" />
        <remoteSelect placeholder="箱号" />
      </el-main>
...
let filterMap: any = computed(() => {
  return tableData.reduce((t, p) => {
    ...
    for (key in p) {
      ...
      if (
        arr.findIndex(
          (item: { value: string; text: string }) => item.value === value
        ) === -1
      ) {
        arr.push({
          value,
